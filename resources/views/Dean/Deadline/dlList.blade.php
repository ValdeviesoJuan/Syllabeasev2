    ...
    <div class="top-0 mt-12">
        <div class="flex flex-col float-left mb-20">
            <div class="py-12 px-12 -mt-12 flex flex-col p-12 md:space-y-0 rounded-xl mx-auto bg-transparent">
                <!-- Move the button inside this container -->
                <div class="mb-5 mt-4 pt-2 text-center">
                    <form action="{{ route('dean.createDeadline') }}" method="GET" class="inline-block">
                        @csrf
                        <button type="submit"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 m-4">
                    ...
                    <div class="role_form p-4 bg-gradient-to-r from-[#FFF] to-[#dbeafe] rounded-xl transform hover:scale-105 transition duration-500 shadow-lg flex flex-col justify-between w-full">
                        <div>
                        ...
                        <div class="text-center mt-6">
                            <form action="{{ route('dean.editDeadline', $dl->dl_id) }}" method="GET" class="inline-block">
                                @csrf
                                <button type="submit" class="btn btn-danger px-14 text-white bg-blue font-semibold hover:bg-[#2563eb] shadow-lg p-1 rounded-lg">Edit</button>
                            </form>
                            <form action="{{ route('dean.destroyDeadline', $dl->dl_id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
