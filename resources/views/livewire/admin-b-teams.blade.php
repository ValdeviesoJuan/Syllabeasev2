<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="flex flex-wrap items-center gap-3 mb-4">
        ...
        <select wire:model="filters.department_id" class="border cursor-pointer focus:outline-none focus:border-blue rounded p-1 w-[16%]" placeholder="Department">
            <option value="" class="">Department (All)</option>
            @foreach($departments as $department)
            <option value="{{$department->department_id}}">{{$department->department_code}}-{{$department->department_name}}</option>
            @endforeach
        </select>
        ...
        <tbody>
            ...
            <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-white' : 'bg-[#e9edf7]' }} bg-white border- dark:bg-gray-800 dark:border-gray-700 hover:bg-gray4 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4 flex">
                    ...
                    <form action="{{ route('admin.destroyBTeam',$bgroup->bg_id) }}" method="Post">
                        @csrf
                        @method('DELETE')
