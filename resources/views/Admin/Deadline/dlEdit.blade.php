...
<div class="flex flex-col justify-center mb-20">
    <div class="relative mt-[100px] flex flex-col bg-gradient-to-r from-[#FFF] to-[#dbeafe] p-12 px-8 md:space-y-0 rounded-xl shadow-lg p-3 mx-auto border border-white bg-white">
        ...
        <form action="{{ route('admin.updateDeadline', $deadline->dl_id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="m-4">
                <div>
                    <label for="syll">Syllabus Deadline</label>
                </div>
                <input type="datetime-local" name="dl_syll" id="syll" class="px-1 py-[6px] w-full border rounded border-gray" value="{{ old('dl_syll', $deadline->dl_syll) }}" required>
            </div>
            <div class="m-4">
                <div>
                    <label for="tos_midterm">TOS Midterm Deadline</label>
                </div>
                <input type="datetime-local" name="dl_tos_midterm" id="tos_midterm" class="px-1 py-[6px] w-full border rounded border-gray" value="{{ old('dl_tos_midterm', $deadline->dl_tos_midterm) }}" required>
            </div>
            <div class="m-4">
                <div>
                    <label for="tos_final">TOS Final Deadline</label>
                </div>
                <input type="datetime-local" name="dl_tos_final" id="tos_final" class="px-1 py-[6px] w-full border rounded border-gray" value="{{ old('dl_tos_final', $deadline->dl_tos_final) }}" required>
            </div>
            <div class="m-4">
                <div>
                ...
                <select name="college_id" id="college" class="select1 w-full px-1 py-[6px] border rounded border-[#a3a3a3]" required>
                    @foreach ($colleges as $college)
                        <option value="{{ $college->college_id }}"
                            {{ old('college_id', $deadline->college_id) == $college->college_id ? 'selected' : '' }}>
