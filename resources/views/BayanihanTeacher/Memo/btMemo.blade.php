...
<div class="mt-16 p-4 shadow bg-white border-dashed rounded-lg dark:border-gray-700">
                ...
                @forelse($memos as $memo)
                    ...
                    <tr>
                        <td colspan="3" class="p-0">
                            <div onclick="handleRowClick(event, '{{ route('bayanihanteacher.memo.show', $memo->id) }}', {{ $memo->id }})"
                                class="teacher-memo-row grid grid-cols-3 items-center rounded shadow-sm cursor-pointer px-2 py-2 transition"
    ...
    <div id="tileView" class="hidden grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($memos as $memo)
            ...
            <div onclick="handleRowClick(event, '{{ route('bayanihanteacher.memo.show', $memo->id) }}', {{ $memo->id }})"
                class="teacher-tile-memo p-4 border rounded-lg shadow cursor-pointer transition relative group overflow-hidden"
                data-memo-id="{{ $memo->id }}"
                style="border: 2px solid {{ $borderHex }};"
            >
                {{-- Title --}}
...
<script>
    function handleRowClick(event, url, id) {
        ...
        // Save memo as read
        const readMemos = JSON.parse(localStorage.getItem("teacherReadMemos") || "[]");
        if (!readMemos.includes(String(id))) {
            readMemos.push(String(id));
            localStorage.setItem("teacherReadMemos", JSON.stringify(readMemos));
        }
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Table rows
        document.querySelectorAll(".teacher-memo-row").forEach(row => {
        ...
        // Tile view
        document.querySelectorAll(".teacher-tile-memo").forEach(tile => {
