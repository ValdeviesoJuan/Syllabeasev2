...
document.addEventListener("DOMContentLoaded", () => {
    ...
    listeners: {
        move(ev) {
            ...
            // New dimensions from resize
            const w = ev.rect.width;
            const h = ev.rect.height;

            tgt.style.width = `${w}px`;
            tgt.style.height = `${h}px`;

            // Optional: update position via transform
            ...
            // Clean up transform (optional)
            tgt.style.transform = '';
            delete tgt.dataset.dx;
            delete tgt.dataset.dy;

            // Optionally persist width/height to localStorage or a layout state object
        }
    },
